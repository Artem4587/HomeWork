...
class MobilePhone {
private:
    int ram;           // Объём оперативной памяти (целое число, в МБ)
    double displaySize;  // Размер экрана (в дюймах)
    string model;       // Модель телефона 
...
public:
    ...
    double getDisplaySize() const { return displaySize; }
...
int main() {
    setlocale(LC_ALL, "RU");
    const int MAX_PHONES = 10; // Максимальное количество телефонов в массиве
    ...
    do {
        ...
        case 2: {
            if (phoneCount < MAX_PHONES) {
                ...
                cin >> displaySize;
                cin.ignore();  // Очистить буфер после cin >> displaySize
                cout << "Введите модель: ";
                getline(cin, model);
                cout << "Введите ОС: ";
                getline(cin, os);
